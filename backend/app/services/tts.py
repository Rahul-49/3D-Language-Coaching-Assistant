...
def _run_rhubarb(wav_path: str) -> List[Dict]:
    ...
    try:
        ...
        print(f"[Rhubarb] ERROR: Rhubarb executable not found at '{rhubarb_path}'. Please check your RHUBARB_PATH environment variable or ensure it's in your system's PATH.")
        return []
...
def synthesize_tts(text: str):
    ...
    if _HAS_EDGE_TTS:
        ...
        # This format is WAV, which is exactly what Rhubarb needs.
        fmt = "riff-16khz-16bit-mono-pcm"
        try:
            async def _gather(txt: str) -> bytes:
                communicate = edge_tts.Communicate(txt, voice=voice)
                chunks = []
                # Pass the format directly to the stream method
                async for chunk in communicate.stream(fmt=fmt):
                    if chunk["type"] == "audio":
            ...
            # The audio bytes are already in WAV format
            audio_bytes = asyncio.run(_gather(text))
            
            with open(tmp_wav, 'wb') as f:
                f.write(audio_bytes)

            visemes = _run_rhubarb(tmp_wav)
            os.remove(tmp_wav) # Clean up temp file
